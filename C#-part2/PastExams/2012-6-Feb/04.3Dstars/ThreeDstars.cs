...
class ThreeDstars
{
    ...
    {
        var sortedDict = starType.OrderBy(x => x.Key);

        Console.WriteLine(starCount);
        foreach (var star in sortedDict)
        {
    ...
    private static void FindSingleStar(int currWidth, int currHeight, int currDepth)
    {
        ...
        bool isStar = currChar == cube[currWidth - 1, currHeight, currDepth] &&
